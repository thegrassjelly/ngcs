 ...
 from atapOfficer where ATAPID = 0 and creatorID=@CreatorID";
        ...
        public void generateAmountOfBond()
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = @"select sum(TotalBondPremium) as AmountOfBond,sum(RemainingBondPremium) as TotalAmountPayable  from ATAPOfficer where creatorID=@CreatorID and ATAPID = 0";
                cmd.Parameters.AddWithValue("@CreatorID", Helper.userID);
 ...
 string to, string agency, string agencyCode, string amountOfBond, string datePrepared, string totalAmountPayable, string approvedBy,string attachment)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                cmd.Connection = con;
                cmd.CommandText = "update ATAPOfficer set ATAPID = @ATAPID where ATAPID = 0 and CreatorID = @CreatorID";
